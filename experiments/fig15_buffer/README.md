# fig15_buffer — Fig. 15: buffer-size sensitivity (R7)
...
Phases: UniNDP baseline + fc_bert-72 mapping pool (shared with Fig 10) ->
...
recorded in `results/fig15_buffer/latest_run_dir.txt`).

Runtimes: smoke `~2 s` with the pool present (`~1 min` cold);
full `<1 min` with the fig10 pool present measured (5-variant eval ~20 s at
